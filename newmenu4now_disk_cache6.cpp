...
std::vector<std::string> findBinImgIsoMdfFiles(const std::string& directory, const std::vector<std::string>& extensions);
//	bools
...
void generateCacheForDirectory(std::vector<std::string>& isoFiles, std::vector<std::string>& binImgMdfFiles, std::string& directoryPath);
void getFilesByExtension(const std::string& directoryPath, std::vector<std::string>& files, const std::string& extension);
void loadBinImgIsoMdfCacheFromFile();
void saveBinImgIsoMdfCacheToFile();
...
std::unordered_map<std::string, std::vector<std::string>> cache; // Declare cache at the beginning

std::vector<std::string> isoFiles; // Declare isoFiles at the beginning
...
std::string getBinImgIsoMdfCacheFilePath() {
    ...
    if (xdgCacheHome) {
        return std::string(xdgCacheHome) + "/isocache_bin_img_mdf.txt";
    } else {
        const char* home = std::getenv("HOME");
        if (home) {
            return std::string(home) + "/.cache/isocache_bin_img_mdf.txt";
        } else {
            // Fallback to a default location if HOME is not set.
            return "isocache_bin_img_mdf.txt";
        }
    }
}

void loadBinImgIsoMdfCacheFromFile() {
    const std::string cacheFilePath = getBinImgIsoMdfCacheFilePath();
    std::ifstream cacheFile(cacheFilePath);
    if (cacheFile.is_open()) {
        while (!cacheFile.eof()) {
            std::string directoryPath;
            std::vector<std::string> binImgMdfFiles;
            cacheFile >> directoryPath;
            while (cacheFile.peek() != '\n' && !cacheFile.eof()) {
                std::string file;
                cacheFile >> file;
                binImgMdfFiles.push_back(file);
            }
            binImgMdfCache[directoryPath] = binImgMdfFiles;
        }
        cacheFile.close();
    }
}
...
void saveBinImgIsoMdfCacheToFile() {
    ...
    if (cacheFile.is_open()) {
        for (const auto& entry : binImgMdfCache) {
            cacheFile << entry.first << '\n';  // Write the directory path on a separate line
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(directory)) {
            if (entry.is_regular_file()) {
                ...
                if (std::find(extensions.begin(), extensions.end(), ext) != extensions.end() &&
                    ...
                    (std::filesystem::file_size(entry) >= 10'000'000)) {
...
void generateCacheForDirectory(std::vector<std::string>& isoBinImgMdfFiles, std::string& directoryPath) {
    std::cout << "Enter the directory path to generate cache: ";
    std::cin >> directoryPath;

    // Add code to list the contents of the directory
    std::cout << "Listing contents of " << directoryPath << ":" << std::endl;
...
std::mutex isoFilesMutex;  // Define a mutex for synchronization
...
int main() {
    ...
    std::string choice;
    std::vector<std::string> binImgIsoMdfFiles;
    std::string directoryPath;
    loadBinImgIsoMdfCacheFromFile();
    while (!exitProgram) {
	...
	std::cout << "6. Populate the ISO Cache" << std::endl;
        ...
        for (const auto& entry : std::filesystem::recursive_directory_iterator(path)) {
            if (entry.is_regular_file()) {
                ...
                if (iequals(std::string(extension), ".iso")) {
                    std::string isoFilePath = filePath.string();

                    // Check for duplicates before adding to the vector
                    if (std::find(isoFiles.begin(), isoFiles.end(), isoFilePath) == isoFiles.end()) {
                        isoFiles.push_back(isoFilePath);
                    }
                }
            }
        }

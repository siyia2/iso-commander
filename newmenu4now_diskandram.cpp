...
std::unordered_map<std::string, std::vector<std::string>> isoListingCache;
std::vector<std::string> isoFiles; // Declare isoFiles at the beginning of the function
    std::string directoryPath; // D

void populateCacheForDirectory(const std::string& directoryPath) {
    std::vector<std::string> isoFiles;
    traverseDirectory(directoryPath, isoFiles);
    isoListingCache[directoryPath] = isoFiles;
}

std::string getCacheFilePath() {
    const char* xdgCacheHome = std::getenv("XDG_CACHE_HOME");
    if (xdgCacheHome) {
        return std::string(xdgCacheHome) + "/isocache.txt";
    } else {
        const char* home = std::getenv("HOME");
        if (home) {
            return std::string(home) + "/.cache/isocache.txt";
        } else {
...
void saveCacheToFile() {
    ...
    for (const auto& entry : isoListingCache) {
        cacheFile << entry.first << ' ';
        for (const std::string& isoFile : entry.second) {
            cacheFile << isoFile << ' ';
        }
        cacheFile << '\n';
    }
...
void generateCacheForDirectory(std::vector<std::string>& isoFiles, std::string& directoryPath) {
...
void getISOListings(const std::string& directoryPath, std::vector<std::string>& isoFiles) {
    if (isoListingCache.find(directoryPath) != isoListingCache.end()) {
        isoFiles = isoListingCache[directoryPath];
    } else {
        std::string pathCopy = directoryPath; // Make a non-const copy of the path
        generateCacheForDirectory(isoFiles, pathCopy);
        isoListingCache[directoryPath] = isoFiles;
    }
}
...
int main() {
   ...
   loadCacheFromFile();
	...
	std::cout << " " << std::endl;
		...
		case '6':
		std::system("clear");
       	 	generateCacheForDirectory(isoFiles, directoryPath);
		std::cout << "Press Enter to continue...";
		saveCacheToFile();
        	std::system("clear");
                    break;

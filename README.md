...
High performance `ISO` manager written in pure C++. `ISOs` are mounted under `/mnt/iso_*` format and conversions are stored in their respective source directories. `ROOT` access is essential for `mount`, `umount` and `write` operations.

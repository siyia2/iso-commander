...
void getISOListings(const std::string& directoryPath, std::vector<std::string>& isoFiles) {
    if (isoListingCache.find(directoryPath) != isoListingCache.end()) {
        ...
        // If not in the cache, perform directory traversal and update the cache.
...
void select_and_mount_files_by_number() {
    ...
    // Use the cache to retrieve or populate the ISO listings for the directory.
    getISOListings(directoryPath, isoFiles);

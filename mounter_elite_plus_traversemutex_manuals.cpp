...
int main() {
    ...
    // Main menu option 1 - Submenu
    bool returnToMainMenu = false;
    while (!returnToMainMenu) {
        std::cout << "Submenu - Choose an option:" << std::endl;
        std::cout << "1. List Mode" << std::endl;
    ...
    std::system("clear"); // Clear the screen after returning from the submenu
...
// Function to create a directory under /mnt with the name "iso_isofilename"
bool createMountDirectory(const std::string& mountDir, const std::string& isoFileName) {
    std::string mountPath = mountDir + "/iso_" + isoFileName;

    // Check if the directory already exists
    struct stat st;
    if (stat(mountPath.c_str(), &st) != 0) {
        // Directory doesn't exist, create it
        if (mkdir(mountPath.c_str(), 0755) == 0) {
            return true;
        }
...
void mountISOmanual() {
    std::vector<std::string> isoFilePaths;

    while (true) {
        // Ask the user for ISO file paths using readline
        char* input = readline("Enter ISO file paths separated by spaces (blank line to exit): ");

        if (!input) {
            std::cerr << "Invalid input. Exiting mountISOmanual." << std::endl;
            return;
        }

        std::string isoFileInput(input);
        free(input);

        // Check for a blank line to exit
        if (isoFileInput.empty()) {
            break;
        }

        // Split the input into individual ISO file paths
        std::istringstream iss(isoFileInput);
        std::string isoFilePath;
        while (iss >> isoFilePath) {
            isoFilePaths.push_back(isoFilePath);
        }

        // Define the fixed mount directory
        std::string mountDir = "/mnt";

        for (const std::string &isoFilePath : isoFilePaths) {
            // Extract the ISO file name (basename) without the extension
            std::string isoFileName = basename((char *)isoFilePath.c_str());
            size_t lastDot = isoFileName.find_last_of(".");
            if (lastDot != std::string::npos) {
                isoFileName = isoFileName.substr(0, lastDot);
            }

            // Sanitize the ISO file path and mount point
            std::string sanitizedIsoFilePath = shell_escape(isoFilePath);

            // Create the mount directory
            if (createMountDirectory(mountDir, isoFileName)) {
                // Construct the mount point path
                std::string sanitizedMountPoint = mountDir + "/iso_" + isoFileName;

                // Mount the ISO file using sudo mount -o loop
                std::string mountCommand = "sudo mount -o loop " + sanitizedIsoFilePath + " " + sanitizedMountPoint;
                int mountResult = std::system(mountCommand.c_str());

                if (mountResult == 0) {
                    std::cout << "ISO file '" << isoFilePath << "' mounted at '" << sanitizedMountPoint << "' successfully." << std::endl;
                } else {
                    std::cerr << "Error mounting ISO file: " << isoFilePath << std::endl;
                }
            } else {
                std::cerr << "Error creating mount directory for ISO file: " << isoFilePath << std::endl;
            }
        }
        isoFilePaths.clear();
    }

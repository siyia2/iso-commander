...
void selectForIsoFiles(const std::string& operation, std::atomic<bool>& updateHasRun, std::atomic<bool>& isAtISOList, std::atomic<bool>& isImportRunning, std::atomic<bool>& newISOFound) {
    ...
    while (true) {
        ...
        // Dummy variable unused here
        bool need2Sort = false;
        
        bool validCommand = processPaginationHelpAndDisplay(inputString, totalPages, currentPage, needsClrScrn, need2Sort, isMount, isUnmount, write, isConversion, isAtISOList);
...
void selectForImageFiles(const std::string& fileType, std::vector<std::string>& files, std::atomic<bool>& newISOFound, bool& list) {
    ...
    while (true) {
        ...
        bool validCommand = processPaginationHelpAndDisplay(mainInputString, totalPages, currentPage, needsClrScrn, need2Sort, false, false, false, true, isAtISOList);

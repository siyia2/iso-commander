    ...
    while (true) {
        ...
        if (!isImportRunning.load()) {
            ...
            if (isAtISOList.load() && !isFiltered) {
                ...
                loadAndDisplayIso(filteredFiles, isFiltered, listSubtype, umountMvRmBreak, pendingIndices, hasPendingProcess, currentPage, operation, isImportRunning);
                std::cout << "\n";
...
void selectForIsoFiles(const std::string& operation, std::atomic<bool>& updateHasRun, std::atomic<bool>& isAtISOList, std::atomic<bool>& isImportRunning, std::atomic<bool>& newISOFound) {
    ...
    while (true) {
        ...
        if (needsClrScrn) {
            if (!isUnmount) {
                if (!loadAndDisplayIso(filteredFiles, isFiltered, listSubtype, umountMvRmBreak, pendingIndices, hasPendingProcess, currentPage, operation, isImportRunning))
                    break;
            } else {
        ...
        if (updateHasRun.load() && !isUnmount && !globalIsoFileList.empty()) {
            std::thread(refreshListAfterAutoUpdate, 2, std::ref(isAtISOList), 
                        ...
                        std::ref(hasPendingProcess), std::ref(currentPage), operation, std::ref(newISOFound)).detach();
        }

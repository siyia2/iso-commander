...
void selectForIsoFiles(const std::string& operation, std::atomic<bool>& updateHasRun, std::atomic<bool>& isAtISOList, std::atomic<bool>& isImportRunning, std::atomic<bool>& newISOFound) {
    ...
    while (true) {
        ...
        bool pendingExecuted = handlePendingExecution(inputString, pendingIndices, hasPendingProcess, isMount, isUnmount, write, isFiltered, 
...
void helpSelections() {
    ...
    std::cout << "\033[1;32m4. Tips:\033[0m\n"
			  << "   • Pending items are limited by current filter/unfiltered status\033[0m\n"

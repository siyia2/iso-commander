...
void mountIsoFile(const std::string& isoFile, std::map<std::string, std::string>& mountedIsos) {
    ...
    if (mountedIsos.find(isoFile) != mountedIsos.end()) {
        std::cout << "\e[1;31mALREADY MOUNTED\e[0m: ISO file '" << sanitizeForShell(isoFile) << "' is already mounted at '" << sanitizeForShell(mountedIsos[isoFile]) << "'." << std::endl;
        return;
    }
    ...
    if (!directoryExists(mountPoint)) {
        ...
        // Sanitize the isoFile path before using it in a shell command
        std::string sanitizedIsoFile = sanitizeForShell(isoFile);

        // Mount the ISO file to the mount point
        if (system(("sudo mount -o loop " + sanitizedIsoFile + " " + sanitizedMountPoint).c_str()) != 0) {
            std::perror("Failed to mount ISO file");
        } else {
            std::cout << "ISO file '" << sanitizeForShell(isoFile) << "' mounted at '" << sanitizedMountPoint << "'." << std::endl;
        ...
        std::cout << "ISO file '" << sanitizeForShell(isoFile) << "' is already mounted at '" << sanitizeForShell(mountPoint) << "'." << std::endl;
        mountedIsos[isoFile] = mountPoint;
        std::cout << "Press Enter to continue...";
...
void select_and_mount_files_by_number() {
    ...
    while (true) {
        ...
        std::string input;
        std::cout << "Choose .iso files to mount (enter numbers or ranges separated by spaces or press Enter to exit): ";
        ...
        while (iss >> token) {
            if (token.find('-') != std::string::npos) {
                ...
                int start, end;
                char dash;
                if (rangeStream >> start >> dash >> end && dash == '-') {
                    if (start >= 1 && end <= isoFiles.size() && start <= end) {
                    ...
                    std::cout << "Invalid range format: " << token << ". Please try again." << std::endl;
                }
            } else {
